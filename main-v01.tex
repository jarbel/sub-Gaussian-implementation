...
\author{$_{1}$Julyan Arbel \footnote{Universit\'{e} Grenoble Alpes, Inria, CNRS, Grenoble INP, LJK, 38000 Grenoble, France}\,\,,
$_{2}$Soufiane Atouani\footnote{??}\,\,,
$_{3}$Olivier Marchal\footnote{Universit\'{e} Jean Monnet Saint-\'{E}tienne, CNRS, Institut Camille Jordan UMR 5208, Institut Universitaire de France, Les Forges 2, 20 Rue du Dr Annino, 42000 Saint-Etienne, France.}
}


\title{Characterisation of optimal sub-Gaussian variance proxy\\ and applications to discrete distributions}
\date{}
...
$_{1}$\textsf{soufiiane.atouanii@gmail.com}
$_{2}$\textsf{julyan.arbel@inria.fr},  
$_{3}$\textsf{olivier.marchal@univ-st-etienne.fr}}
%\small
...
\textcolor{red}{Olivier: We should merge the previous two lemmas to state something for two positive zeros on $R_+^*$ for $g''_Y(.,\sigma^2)$. Basically, it says that in this case, the equation $g_Y(\lambda,\sigma^2)=0=g'_Y(\lambda,\sigma^2)$ has at most one solution with $\lambda\in R_+^*$. If it has no zero then $g_Y(.,\sigma^2)$ is always non negative. If it has one solution $(\lambda_c,\sigma_c^2)$ then $\sigma_c^2>Var[Y]$ and $g_Y(.,\sigma^2)$ is non-negative on $R_+$ if and only if $\sigma^2\geq \sigma_c^2$. 
}
\end{comment}
...
In the \textbf{symmetric case}, where the outer masses are equally weighted, we establish two distinct behaviors. When the parameter satisfies $p \geq \tfrac{1}{6}$, the distribution is strictly sub-Gaussian and the optimal variance proxy coincides with the variance,
\[
...
In the \textbf{asymmetric case}, where the mass probabilities at $-a$ and $a$ are not equal, the situation is more intricate. If the central mass satisfies $p_3 \leq 4\sqrt{p_1p_2}$, then an explicit closed-form expression is available:
\[
...
\subsection{Symmetric case of 3-mass distribution}\label{subsec;symmthreepoints}
...
\end{equation}
where $p\in \left( 0,\frac{1}{2}\right)$.
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[scale=1.2]
        ...
        \draw[->] (-2,0) -- (2,0);
        % Masses
        \draw[black, thick] (-1,0) -- (-1,1) node[above, black] {$p$};
        ...
        \node at (1,-0.2) {$a$};
    \end{tikzpicture}
    \caption{Probability mass function of the symmetric 3-mass distribution.}
    \label{fig:three_mass_distribution}
\end{figure}
...
\begin{comment}
\textcolor{blue}{Olivier: We should highlight the fact that the existence of two regimes: one strictly subGaussian and the other not depending on the value of $p$ is something quite remarkable. Also a theorem merging both cases would help the reading.}

\textcolor{red}{\subsubsection{Refinements and comments}
...
\end{equation}
where $p_1, p_2\in \left( 0,1\right) \text{ and since } p_3=0$ corresponds to two masses case  we will consider $p_3 > 0$ and $a>0$. We may define $Y=\frac{1}{a}X$ and use the fact that $\sigma_{\mathrm{opt}}[X]=a \sigma_{\mathrm{opt}}[X]$. Thus, we may restrict to $a=1$. We also may assume $p_2 \geq p_1$.  
...
\textcolor{red}{Olivier: In what sense can this be seen as an application since ti does not seem that we use one of the main result in this section?}\\
\textcolor{blue}{Soufiane: I agree that this section does not directly use any of the main results. I included it because we utilize the function g used in characterization. We can discuss choosing a more appropriate title for this section.}\\
...
\beq
P_\lambda(k) = \frac{e^{\lambda k}}{\underset{j=1}{\overset{N}{\sum}}e^{\lambda j}}, \quad k \in \mathcal{I}.
\eeq
...
\eeq
is the optimal variance proxy, and implies that \( Y \) is strictly sub-Gaussian. Thus \(X\) is strictly sub-Gaussian and its optimal proxy variance has the form \beq \sigma^2_{\mathrm{opt}}[X] = a^2 \frac{N^2 - 1}{12} \eeq 
...
In addition, the package also implements a recent characterization of the optimal variance proxy based on the function 
\[
...
\]
leading to the critical sets $\mathcal{L}_c$ and $\mathcal{S}_c$ introduced in  section~\ref{sec:charac}. 
...
Overall, the software provides a comprehensive and user-friendly environment for exploring sub-Gaussian properties, and serves as a computational companion for researchers applying concentration bounds in Bayesian inference, variational methods, and beyond.
